...
def main():
    ...
    for ds_name in DATASETS:
        ...
        for ax, title in [(ax_rec, f"RECOVAR-{m_name.upper()}"), (ax_phase, f"PhaseNet-{m_name.upper()}")]:
            ax.set_title(f"{ds_name.upper()} Dataset | {title} ROC Curves")
            ax.set_xlabel("False Positive Rate")
            ax.set_ylabel("True Positive Rate (Recall)")
            ax.set_xlim(0.0, 1.0)
            ax.set_ylim(0.0, 1.05)
            ax.grid(True, linestyle="--", alpha=0.7)
            ax.legend(fontsize=9, loc='lower right', title="Mag Bin (AUC)")
            
        fig.tight_layout()
        
        out_png = OUTPUT_DIR / f"roc_curves_by_mag_{ds_name}_model_{m_name}.png"
