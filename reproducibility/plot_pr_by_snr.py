    ...
    for ds_name in DATASETS:
        ...
        for m_name in ["stead", "instance"]:
            ...
            for ax, title in [(ax_rec, f"RECOVAR-{m_name.upper()}"), (ax_phase, f"PhaseNet-{m_name.upper()}")]:
                ax.set_title(f"{ds_name.upper()} Dataset | {title} PR Curves by SNR")
                ax.set_xlabel("Recall")
                ax.set_ylabel("Precision")
                ax.set_xlim(0.0, 1.0)
                ax.set_ylim(0.0, 1.05)
                ax.grid(True, linestyle="--", alpha=0.7)
                ax.legend(fontsize=9, loc='lower left', title="SNR Bin (dB) (AUC)")
                
            fig.tight_layout()
            
            out_png = OUTPUT_DIR / f"pr_curves_by_snr_{ds_name}_model_{m_name}.png"
