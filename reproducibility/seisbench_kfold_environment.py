...
from seisbench_data_generator import SeisBenchDataGenerator
from data_generator import GeneratorWrapper, PredictGenerator


class SeisBenchKFoldEnvironment:
    def __init__(
        self,
        dataset,
        event_dataset,
        ...
        metadata = self._parse_seisbench_metadata(event_dataset, noise_dataset)
    ...
    def _parse_seisbench_metadata(self, event_dataset, noise_dataset):
        eq_metadata = event_dataset.metadata.copy().reset_index(drop=True)
        no_metadata = noise_dataset.metadata.copy().reset_index(drop=True)
        ...
        eq_metadata = self._drop_short_traces(eq_metadata, min_ts, event_dataset)
        no_metadata = self._drop_short_traces(no_metadata, min_ts, noise_dataset)
        ...
        if "source_id" in eq_metadata.columns:
            ...
            eq_metadata["source_id"] = [f"eq_{i}" for i in range(len(eq_metadata))]

        no_metadata["source_id"] = [f"no_{i}" for i in range(len(no_metadata))]

        for col in ("p_arrival_sample", "s_arrival_sample"):
            if col not in eq_metadata.columns:
                eq_metadata[col] = np.nan
            no_metadata[col] = np.nan
    ...
    def _drop_short_traces(self, metadata, min_ts, dataset):
        if len(metadata) == 0:
            return metadata
        npts = self._stored_npts(metadata, dataset).to_numpy()
        resampled = npts
    ...
    @staticmethod
    def _stored_npts(metadata, dataset):
        names = metadata["trace_name"].astype(str)
        bucketed = names.str.contains(r"\$")
        npts = pd.Series(np.nan, index=metadata.index)
        if bucketed.any():
            npts[bucketed] = (
                names[bucketed].str.extract(r":(\d+)$")[0].astype(float).values
            )
        plain = ~bucketed
        if plain.any():
            if "trace_chunk" in metadata.columns:
                chunks = metadata["trace_chunk"].fillna("").astype(str)
            else:
                chunks = pd.Series("", index=metadata.index)
            for chunk in chunks[plain].unique():
                sel = plain & (chunks == chunk)
                with h5py.File(dataset.path / f"waveforms{chunk}.hdf5", "r") as f:
                    g_data = f["data"]
                    npts[sel] = [float(max(g_data[n].shape)) for n in names[sel]]
        return npts
    ...
    def _get_datagen(self, active_chunks=None):
        ...
        datagen = SeisBenchDataGenerator(
    ...
    def _form_kfold_splits(self, random_state=0):
        ...
        if self._n_splits > 1:
            ...
            num_training_chunks = int(len(chunk_list) * (1.0 - self.test_ratio))
    ...
    def _resample(self, chunk_metadata_list):
        ...
        for chunk_metadata in chunk_metadata_list:
            ...
            eq_resampled = eq_chunk_metadata.sample(
                n=target_num_eqs, replace=True, random_state=0
            )
            no_resampled = no_chunk_metadata.sample(
                n=target_num_nos, replace=True, random_state=0
            )
